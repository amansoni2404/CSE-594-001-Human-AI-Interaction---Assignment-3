...
def submit():
    ...
    # csv_content = df_responses.to_csv(index=False)

    # Send email
    subject = f"Sarcasm Detection Responses - {participant_name} (Human Only)"
    body = f"Responses from participant: {participant_name}"
    send_email_with_csv(subject, body, df_responses, EMAIL_ADDRESS, csv_filename=f"{participant_name}_human_only_resp.csv")
